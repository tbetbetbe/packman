...
var commonProtoTemplateInfo = {
  go: {
    templateDir: path.join(__dirname, '..', 'templates', 'commonpb', 'go')
  },
  objective_c: {
    templateDir: path.join(__dirname, '..', 'templates', 'commonpb', 'objc')
  },
  nodejs: {
    templateDir: path.join(__dirname, '..', 'templates', 'commonpb', 'nodejs')
  },
  python: {
    copyables: [
      'README.rst'
    ],
    templateDir: path.join(__dirname, '..', 'templates', 'commonpb', 'python')
  },
  ruby: {
    templateDir: path.join(__dirname, '..', 'templates', 'commonpb', 'ruby')
  }
};
...
ApiRepo.prototype.buildCommonProtoPkgs =
  function buildCommonProtoPkgs(opt_done) {
    ...
    this.languages.forEach(function(l) {
      var buildTasks = [];
      config.commonProtoPkgs.forEach(function(pkgSpec) {
        buildTasks.push(
...
ApiRepo.prototype._checkDeps = function _checkDeps(opts, done) {
  ...
  this.languages.forEach(function(l) {
    ...
    if (_.has(that.overridePlugins, l)) {
...
ApiRepo.prototype._makeProtocFunc = function _makeProtocFunc(opts, language) {
  ...
  var callProtoc = function callProtoc(protoPath, done) {
    ...
    if (language === 'go') {
      ...
      console.log('What are opts at this point %j', opts.buildCommonProtos);
      if (!opts.buildCommonProtos) {
        args.push('--grpc_out=' + outDir, '--plugin=protoc-gen-grpc=' + pluginBin);
      }
      args.push(protoPath);
    }
