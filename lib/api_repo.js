    ...
    function _buildProtos(name, version, language, done) {
      ...
      var findOutputs = function findOutputs(err) {
        if (err) {
          ...
          var stripRoot = function stripRoot(err, outputs) {
            if (err) {
              done(err);
            } else {
              var stripped = _.map(outputs, function(x) {
      ...
      if (language === 'java') {
        var dstResourceDir = path.join(langTopDir, 'src', 'main', 'resources');
...
ApiRepo.prototype._findProtos = function _findProtos(name, version, done,
  ...
  var actOnProtos = function actOnProtos(foundProtos, next) {
    if (onProto) {
